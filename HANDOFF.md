...
| | |
|---|---|
| Bundle id / package | `com.altixcode.syncprompt` |
| App Store Connect app | `6811948361` (version 1.0, `PREPARE_FOR_SUBMISSION`) |
| In-app purchase | `com.altixcode.syncprompt.removeads` — ASC id `6811948893`, state `MISSING_METADATA` |
| RevenueCat project | `proj3ebb98d1` |
| RevenueCat apps | iOS `appcba3650df0` · Android `app68200997c9` |
...
- **Interstitial**: app/prompter.tsx — after a take reaches the library. Pacing lives in `adPolicy.ts`: the first completion
...
- **Tests**: 70 passing, typecheck clean.
...
2. **Write and upload the store listing.** This app has no description and no
   ...
   `Dev/scripts/store-metadata.json`; add an entry for SyncPrompt and upload it with
...
4. **Upload the IAP review screenshot.** The in-app purchase
   (`com.altixcode.syncprompt.removeads`) is localized and priced at $3.99, and sits at
   `MISSING_METADATA` for want of one screenshot:

   ```bash
   asccli iap-review-screenshot upload --iap-id 6811948893 --file <path-to-png>
   ```
...
2. **RevenueCat — Apple credentials.** `appcba3650df0` (the App Store app in project
   `proj3ebb98d1`) has no Apple credentials, so App Store purchases cannot be
   validated. This needs an interactive Apple ID sign-in with 2FA:

   ```bash
   rc setup apple appcba3650df0
   ```
...
3. **Create the GitHub repo first — there is none.** `git remote -v` is empty, so
   ...
   and nothing is backed up. Create `AltixCode/syncprompt` and push, then add
   ...
   Set each with `gh secret set <KEY> --repo AltixCode/syncprompt`. The RevenueCat
   public SDK keys are fetchable — `rc api GET "/projects/proj3ebb98d1/apps/appcba3650df0/public_api_keys"`
   — the AdMob ones come from step 1.

4. **The legal URLs are wrong and they 404.** `src/config/legal.ts` points at
   `https://www.hushtunnel.com/legal/syncprompt-privacy` — **hushtunnel.com**, a leftover from the HushTunnel
...
5. **Play Console — create the app.** `com.altixcode.syncprompt` is not recognised by
...
../scripts/verify-app.sh syncprompt com.altixcode.syncprompt   # full build + device verification, both platforms
```
